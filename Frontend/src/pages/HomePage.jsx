...
// get list of possible friends
 ...
 useEffect(()=>{
  ...
  if(outgoingFriendReqs && outgoingFriendReqs > 0){
    outgoingFriendReqs.forEach((req)=>{outgoingIds.add(req.recipient_id);
  })
    // update the state
...
//  now what we have these varaibles we need to display them 
  ...
  return (
    <div className='grid grid-rows-10 grid-cols-10 grid-flow-col auto-cols-min min-h-[100%]' >
      {/* Navbar */}
...
{/* recommened users header */}
      ...
      {/* recommended users content */}
        <div className='grid grid-rows-subgrid grid-cols-subgrid row-span-full col-span-9 items-center grid-flow-col row-start-6'>
        {
          loadingRecommended?(
